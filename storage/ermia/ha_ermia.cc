...
static int ermia_init_func(void *p) {
  ...
  // XXX(jianqiuz): Add knob for ermia handlerton
  // XXX(khuang): Add knob for commit pipeline
  ...
  // FIXME(tzwang): Hard-coded for now
  ...
  ermia::config::group_commit_timeout = 1;
...
int ha_ermia::write_row(uchar *buf) {
  ...
  // FIXME(jianqiuz): Handle the AUTO INCREMENT, we need to implement ha_ermia::index_last()
  ...
  // FIXME(jianqiuz): The AUTO_INCREMENT does not have effect.
  ...
  for (uint key = 0; key < table->s->keys; key++) {
    ...
    // FIXME(jianqiuz): How can we recycle this buf in this transaction.
...
#if 0 
    // TODO(tzwang): add upsert support
    // XXX(jianqiuz): The following implementation does nothing (but add overhead) to the current
    ...
    if (0 /* Intentionally skip the code here */  && !strcmp("PRIMARY", full_key->name) && full_key->actual_key_parts > 1) {
        ...
        for (auto idx = 0; idx < full_key->actual_key_parts; idx++) {
          ...
          // TODO(jianqiuz): The logic here is a bit complicated. Need refine in the future
...
int ha_ermia::update_row(const uchar *old_data, uchar *new_data) {
  ...
  // FIXME(jianqiuz): Keylenth is incorrect, for VARSTR
  ...
  // TODO(tzwang): should we support no-index cases?
  ...
  // Note (tzwang): trans_rollback_stmt defined at sql/transaction.cc:564 which
  ...
  // TODO(tzwang): return more proper error codes for SI_CONFLICT and SERIAL
...
int ha_ermia::index_read_map(uchar *buf, const uchar *search_key,
  ...
  if (find_flag == HA_READ_KEY_EXACT) {
    ...
    if (table->s->key_info[active_index].flags & HA_NOSAME) {
        ...
        idx->GetRecordMulti(et->tx, rc, *key, et->result_vector);
        if(rc._val == RC_FALSE || et->result_vector.empty()) {
            et->result_ptr = 0;
            DBUG_RETURN(HA_ERR_KEY_NOT_FOUND);
        }
        auto first_result = et->result_vector[0];
        memcpy(buf, first_result.data(), first_result.size());
        et->result_ptr = 1;
    }
  } else {
    // TODO: Write a callback ? maybe to figure out how to do range select.
    rc = idx->Scan(et->tx, *key, NULL, cb, et->arena);
    if (rc._val == RC_FALSE) {
      et->result_ptr = 0;
      DBUG_RETURN(HA_ERR_KEY_NOT_FOUND);
    } else {
      et->result_vector = cb.result();
      if (!et->result_vector.empty()) {
        auto first_result = et->result_vector[0];
        memcpy(buf, first_result.data(), first_result.size());
        et->result_ptr = 1;
      }
...
int ha_ermia::index_last(uchar *data) {
  ...
  // FIXME(jianqiuz): Suppose the index is PRIMARY is not correct.
  ...
  // TODO(jianqiuz): Currently this will scan all the record from masstree and then we choose
...
/**
  ...
  sql_update.cc
  */
// FIXME(jianqiuz): Use correct ordering for rnd_init (do a masstree scan)
int ha_ermia::rnd_init(bool) {
  ...
  for(ermia::OID oid = 0; oid < himark; oid++) {
    ...
    // TODO(jianqiuz): Can I just SKIP it?
    if(ptr.asi() & ermia::fat_ptr::ASI_DIR) {
       // TODO(jianqiuz): Add read logic
       oids.clear();
       ermia::oidmgr->oid_get_dir(td->GetTupleArray(), oid, oids);
       continue;
    } else {
        oids.clear();
        oids.push_back(oid);
...
ermia::varstr *ha_ermia::Encode(ermia::str_arena *arena, const uchar *buf, const uint len,
  ...
  if (!encode_type) {
    ...
    if (len == 2) {
      ...
      // FIXME(jianqiuz): Why we need to do bitwise operation to get the int32? I think a direct conversion is okay
